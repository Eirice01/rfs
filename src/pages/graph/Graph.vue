<template>
    <div id="graph">
      ...
      <!--节点编辑-->
      <node-edit ref="nodeEdit" @selectNodeImageData="selectNodeImageData"></node-edit>
      <!--拆分节点-->
      <split-node></split-node>
      <!--合并节点-->
      <merge-node></merge-node>
      <!-- 共同关联 -->
      <commonRelation-set v-if="readyRelation"></commonRelation-set>
      <!--右键添加任务-->
...
<script>
    ...
    import CommonRelationSet from "@/components/modals/CommonRelationSet"
    ...
    export default {
        ...
        methods: {
          //更新图中对象属性
          setModelTars(){
            this.$refs.AdvancedSearch.setNodePro();
          },
          //手动改变标签指标初始
          setlabel(){
          ...
          //手动改变标签指标关闭
          setCloselable(){
            this.$refs.graphicalquota.watchLablClose()
          },
          //获取指标框状态
          getSliderLvl(){
          ...
          initRightCon: function () {
            ...
            setTimeout(function () {
              ...
              if(!_.isEmpty(currentModel) && !_.isEmpty(currentModel.graphId)){
                if(_.isEmpty(currentModel.data.groups)){
          ...
          changeWidthValue(){
            ...
            this.$refs.bottomTaskbar.refreshEcharts();
          },
          //节点编辑获取通用图标和实体图标
          getDefaultIconData(){
            this.$refs.nodeEdit.getDefaultIcon();
          },
          changeMenuShow(value){
            this.ready = value;
          }
      },
...
<style>
  ...
  #graph .ivu-tabs-content {
    height: 100%;
    width: 100%;
    position:relative;
  }

  #graph .ivu-tabs-tabpane {
    height: 100%;
    width: 100%;
  }
  ...
  #graph .tabs-style.ivu-col.ivu-col-span-12 {
    width: 100%;
  }
