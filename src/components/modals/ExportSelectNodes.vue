  ...
  export default {
    ...
    data() {
      const taskdatePass=(rule,value,callback)=>{
          ...
          }if(value.length==0 &&this.$store.state.isExportSelectNodes){
    ...
    methods: {
     ...
     //导出节点信息
      ...
      getSelectNodesInfo(){
        ...
        //节点property集合
        var propertys = [];
        let propertydata = _.map(_.map(nodes, function (node) {
          return node.data()
        }));
