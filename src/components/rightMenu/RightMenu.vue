  ...
  export default {
    ...
    data (){
      return {
        ...
        clickEdge:false,
        selectedNodeNum:0,
        selNodeLock:false,//节点是否锁定
        selNodeAsh:false,//节点是否置灰
        menuConfig:[],
        menuConfig1:[
          {
            name:"标记",
    ...
    methods:{
      ...
      nodesShowMenu(name){
        switch(name){
          case "拆分节点":
           if(this.selectedNodeNum == 1 && !this.selNodeLock && !this.selNodeAsh) { //选中的点为一个并且不锁定不置灰才可显示该功能
              return true;
           } else {
              return false;
           }
          ...
          case "合并已选":
            if(this.selectedNodeNum == 2 && !this.selNodeLock && !this.selNodeAsh){
              return true;
            } else {
              return false;
            }
          ...
          case "添加关系":
            if(this.selectedNodeNum ==1 && !this.selNodeLock && !this.selNodeAsh){
              return true;
            } else {
              return false;
            }
          ...
          case "节点编辑":
            if(this.selectedNodeNum > 0 && !this.selNodeLock && !this.selNodeAsh){
              return true;
            } else {
              return false;
            }
        ...
        if(!this.currentGraphId) {
          ...
          container.oncontextmenu = (e) => {
            ...
            if(!isCloseCase){//结案任务不显示右键菜单
              ...
              let seledNodes = cy.nodes(":selected");
              ...
              if(cy.nodes(":selected").length !== 0 && _this.clickContainer){
                ...
                if(cy.nodes(":selected").length === 1){ //选择节点只有一个时以节点位置为中心点显示右键菜单
                  ...
                  this.selNodeLock = seledNodes.data().isLock == 1;  //选中的一个点是否锁定
                  //选中的一个点是否置灰
                  this.selNodeAsh = seledNodes.hasClass('a-nodehightlight') ||　seledNodes.hasClass('ｂ-nodehightlight')
                  ...
                  //选中的所有点都是锁定的
                  this.selNodeLock = seledNodes.every(function (item,index,array) {
                    return item.data().isLock == 1;
                  });
                  //选中的所有点都是置灰的
                  this.selNodeAsh = seledNodes.every(function (item,index,array) {
                    return item.hasClass('a-nodehightlight') ||　item.hasClass('ｂ-nodehightlight')
                  })
      ...
      deleteLabels(){
        ...
        //单节点操作时有此判断
        if(collectionNodes.length == 1 && list[0].labels.length==0){
      ...
      clickHandler(type){
        ...
        let cy = currentGraphModel.cy;
        let nodes;
        if(currentGraphModel.collectionNodes && currentGraphModel.collectionNodes.length === 0){
          nodes = cy._private.curNode;
        } else {
      ...
      removeGroup(){
        if(!this.hasGroup){
          this.menuConfig.forEach(v => {
            if(v.name == "聚合" && v.children.length > 3){
