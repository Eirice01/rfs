<template>
  <div :class="isShowSocialExponent ? 'show' : 'hide'" id="socialExponent">
    <div slot="close" class="cls-close">
      ...
      <span id="question-groupmodel" style="margin-right: -325px;font-size:16px;cursor: pointer" >
    ...
    <div class="socialExponent-header">
      <Row style="line-height:34px;margin-top:5px;padding-left:6px">
        ...
        <Input v-model="exponentSearchContent" @on-keyup="searchNodeExponent(exponentSearchContent)" icon="ios-search" size="small" placeholder="搜索名称..."
               style="width: 200px"></Input>
      </Row>
    </div>

      <div style="padding:6px 17px 10px 17px; overflow: auto">
      ...
      <div class="pageList">
        ...
        <Page style="float: right" @on-change="changePage" :current="currentPage" :page-size="pageSize" :total="nodeTotal" size="small"></Page>
      </div>
    <div id="modelInfo">
    ...
    export default {
      ...
      methods: {
        ...
        showSocialExponent(typeGroup) {
          ...
          if(_.isEmpty(typeGroup)){
           ...
           // this.closeSocialExponent()
            ...
            this.currentPage = 1;
            this.nodeList = [];
            let sorted = document.querySelector("#socialExponent .ivu-table-sort i.on");
            if(sorted){
              this.isfilter=false;
              sorted.click();
            }
        ...
        closeSocialExponent(){
          ...
          //初始化排序图标
        let els=document.getElementsByClassName("on")
            // for(let i=0;i<els.length;i++){
            //   $(els[i]).css({color:"#c5c8ce"})
            // }
        ...
        searchNodeExponent(content) {
          ...
          if(content !== ""){
            ...
            //存储过滤出的来原始数据
            this.filterTotalNodes=filterNodeList;
          } else {
            this.allNodeList = JSON.parse(JSON.stringify(this.oriTotalNodes));
          }
          this.nodeTotal = this.allNodeList.length;
        ...
        sortHandler({column,key,order}){
          ...
          if(order == "desc"){
            ...
            if(this.isfilter){
              this.nodeList=[];
              this.filterTotalNodes
              this.allNodeList = JSON.parse(JSON.stringify( this.filterTotalNodes));
              let nodeList = [];
