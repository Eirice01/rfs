<template>
  <div id="analytical-model">
    <ul id="tasks-list-container">
      <li v-for="(model,index) in analyticalModelList" :key="model.rid" @click="currentModelInfo($event,model)" style="cursor: pointer" class="cls-taskModels">
        <div  class="task-analytical-model"  :class="model.rid===currentindex ?  'menumodel': model.rid===hasBorder ? 'border-menumodel' : ''"   :id="model.rid">
          <Row :gutter="8">
            <Col span="9">
            <div class="model-img">
              <img @mouseenter="mouseEnter($event)" @mouseleave="mouseLeave($event)" @click="changeGraphModel($event,model)" class="analytical-model-img f-csp" title="查看结果"
                ...
                <li class="model-content-li" style="margin-top: 3px">
                  <div id="cls-modelinfo">
                   ...
                   <span >{{getModelNodeSize(model.inputCount, model)}}</span>
                    ...
                    <img src="../../../assets/img/outpro.png" style="margin-right: 5px;margin-left:10px" @click="selectIsInputNode(1,$event,index)" class="cls-Icon">
                    </Tooltip>
                    <span >{{getModelNodeSize(model.outputCount, model)}}</span>
                  </div>

                </li>
                <li class="model-content-li" :class="{contentli4:model.rid===currentindex || model.rid===hasBorder}" id="toolscontent" style="position: absolute;  right: -11px; margin-top: 0px;" >
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //改变当前模型的样式条件
      currentModelInfo(e, model) {
        e.stopPropagation();

        // this.currentindex = index;
        this.hasBorder = model.rid;
        //切换rid
     ...
     //点击图片模型切换
      changeGraphModel(e, model){
        ...
        if((_.isEmpty(currentGraphModel.data.nodes) && _.isEmpty(currentGraphModel.data.edges))
          ...
          if(this.$store.state.isSnapshotConfirm){//图有变化弹出是否保存快照提示框
            ...
            //切换图形显示加载图
            this.$store.commit("graphLoad",true);
            this.$store.state.modelrname = model.rname;
            e.stopPropagation();
            this.currentindex = model.rid;
            this.hasBorder = model.rid;
            this.$store.commit("modelRid",model.rid);
            this.viewModelResult(model, e)
          }
        }
      },
      cancelTask(model,e){
            ...
            this.$http.request({
              ...
              success: (data) => {
                if (data.code === 200 && data.data === "1") {
                  this.refreshAnalyticalModelList();
                  //显示当前模型结果，删除后显示任务数据
      ...
      modelCopy(model){
        let rid = model.rid;
        let baseDic=this.$store.state.queryBaseDictionData;
        let manyPro = baseDic.manyPro;
        this.$http.request({
          method: 'get',
          params:{rid},
          url: URL.viewParams,
          success:(data) => {
            if (data.code === 200) {
              let res = data.data;
             // let nodes = [];
              let paramNodesList = res.accountVo.parpamVo;
          _.forEach(paramNodesList,function (item)  {
            let typeVo=_.find(baseDic.nodePro,{"label":item.type});
            item.typeName=_.isEmpty(typeVo)?item.type:typeVo.name;
            item.name=_.split(item.value, '\n')[0];
            item.id=item.name;
          });
          res.dicCode = model.rtype;
              let dicVo =_.find(manyPro,{"dicCode":res.dicCode});
          if(!_.isEmpty(dicVo) && !_.isEmpty(dicVo.val2)){
            res.val2 = dicVo.val2;
          }
 /*         let nodeNames = _.split(paramNodes.value, '\n');
          let type = paramNodes.type;
              ...
              this.$refs.createStatistics.showModal(res, '模型复制', paramNodesList, 'copy');
            }
          },
          error: (data) => {
            this.$Message.warning('请求数据失败！');
            ...
            this.$http.request({
              ...
              success: (data) => {
                if (data.code === 200) {
                  let currentGraphModel = this.$store.state.currentGraphModel;
                  this.queryTaskModelResult(currentGraphModel);
                  this.$Message.success("任务重发成功：" + model.rname);
                  console.log(model.index)
                } else if (data.code === 500) {
                  this.$Message.success("服务器异常，请稍后重试!")
                } else {
                  this.$Message.warning('任务重发失败');
                }
              },
              error: (data) => {
                this.$Message.warning('任务重发失败');
