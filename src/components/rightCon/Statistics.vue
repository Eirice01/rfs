    ...
    <ul>
      <li class="statistics-info statistics-li f-cbli">
        ...
        <div class="modelinfos">
          ...
          <p>亲密度越高，则团体之间联系越紧密</p>
          <div class="mainTitle">稳固度</div>
          <p>稳固度越高，则团体之间的关系越稳定</p>
        </div>
      </li>
      <li v-if="isLoadStatisticsAndAnalyzeCon" class="statistics-li f-cbli">
        <div id="nodeGroup-container" class="statistics-container">
          ...
          <div class="main-clsf"  @click="allCheck" :title="ischeckCls?'对象全勾选':'对象全部去勾选'">
          ...
          <ul>
            <li v-for="(group,index) in nodesTreeData" class="group-li f-csp">
              <span @mouseenter="addHighlight" @mouseleave="cancelHighlight" style="cursor: pointer;display: inline-block;">
                ...
                <span @click="treeGroupUnfold(nodesTreeData, group, 'node')" :title="group.name" class="f-csp statistics-group-span">{{group.name}}</span>
                <div class="cls-checklist" @click="checkList(index,group.type,group.name)" :title="group.name+'勾选'">
      ...
      <li v-if="isLoadStatisticsAndAnalyzeCon"  id="edgeGroup" class="statistics-li f-csp">
        <div id="edgeGroup-container" class="statistics-container">
          ...
          <div class="main-clsedge"  @click="alledgesCheck" :title="isedgecheck?'关系全勾选':'关系全部去勾选'">
          ...
          <ul>
            <li v-for="(group,index) in edgesTreeData" class="group-li">
               <span @mouseenter="addHighlight" @mouseleave="cancelHighlight" style="cursor: pointer;display: inline-block;">
                ...
                <span @click="treeGroupUnfold(edgesTreeData, group, 'edge')"  :title="group.name" class="f-csp statistics-group-span">{{group.name}}</span>
                  <div class="edge-checklist" @click="checkEdges(index,group.type)" :title="group.name+'勾选'">
    ...
    methods: {
      ...
      allCheck(){
        ...
        if(this.ischeckCls==false){
          ...
          // 显示所有的点
          cy.nodes().show();
          // 根据勾选的边属性选择性的显示图上的边  --  $("#edgeGroup-main .edge-Current") 未勾选的关系
          let length = $("#edgeGroup-main .edge-Current").length;
          // 未勾选的关系长度为0  说明所有的边关系都是勾选状态  展示所有的边即可
          if(length == 0){
            cy.edges().show();
          }else {
            let innerName=[];
            let innerCode=[];
            $("#edgeGroup-main .edge-Current").each(function () {
              // 例如：通联关系
              innerName.push($(this).parent().children().eq(1).html());
            });
            _.filter(this.edgesTreeData,function (group) {
              if(innerName.indexOf(group.name) > -1){
                innerCode.push(group.type);
              }
            });
            _.filter(cy.edges(),function (edge) {
              if(innerCode.indexOf(edge.data().type) > -1){
                //右侧统计隐藏关系点添加标识
      ...
      checkList(index,type,name){
        ...
        // 判断为CurrentChecklist勾选的样式
        if(currentList.classList.contains('CurrentChecklist') === true){  // 选中事件
          // 将灰显的样式取消
      ...
      statisticsSelectNode(selectItem) {
        ...
        if(!_.isEmpty(selectItem)) {
          ...
          if(selectItem.isNode()){
            ...
            //显示关联节点
            if(selectItem.rightMenuNode) {
              this.showSingleInfo(selectItem);
              selectItem.rightMenuNode = false;
            }else {
              this.initOverallStatistics();
            }
          }else {
      ...
      initContainer(nodes, edges) {
        ...
        _.each(nodeTypeGroup, function (children, nodeType) {
          ...
          _this.$store.commit("getModelTar",_this.setTar);
          _this.$emit('handleModelTars')
        });
        this.nodesTreeData = this.allNodesTreeData;
